# -*- coding: utf-8 -*-
"""
Image Collage maker - tool to create image collages
Author: Abdul Basit
"""
...
def make_collage(images, filename, width, init_height):
    ...
    for i in images:
        ...
        for ii in images_list:
            ...
            if x > width:
                img_width_lines.append((float(x) / width, images_line))
                images_line = []
                x = 0
        ...
        if len(img_width_lines) <= 1:
            break
        if any(map(lambda c: len(c[1]) <= 1, img_width_lines)):
            # reduce `init_height`
            init_height -= 10
        else:
            break
    ...
    # get output height
    out_height = 0
    for img_width, imgs_line in img_width_lines:
        if imgs_line:
            out_height += int(init_height / img_width) + margin_size
    if not out_height:
    ...
    for img_width, imgs_line in img_width_lines:
        if imgs_line:
            ...
            for img_path in imgs_line:
                ...
                k = (init_height / img_width) / img.size[1]
                if k > 1:
                    img = img.resize((int(img.size[0] * k), int(img.size[1] * k)), Image.ANTIALIAS)
                else:
                    img.thumbnail((int(width / img_width), int(init_height / img_width)), Image.ANTIALIAS)
                if collage_image:
                    collage_image.paste(img, (int(x), int(y)))
                x += img.size[0] + margin_size
            y += int(init_height / img_width) + margin_size
